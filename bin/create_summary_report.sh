...
All temp ID changes are listed in '"${report_file}.report_duplicate_sample_id_temp_changes"'
SNP probes which target multiple regions of the genome have been identified and removed. These SNP IDs are found in '"${multimapping_snp_file}"'
All allele have been converted to Illumina top strand" >> README_summary_report

echo -e "\nA summary of the pipeline is given below:" >> README_summary_report

echo -e "\nTotal number of SNPs prior QC: $total_snp_number_after_qc" >> README_summary_report

echo -e "\nTotal number of Samples prior to QC: $total_sample_number_after_qc" >> README_summary_report

echo -e "\nTotal number of mulimapping SNPs removed: $number_of_multimapping_snps		...see file: '"${multimapping_snp_file}"' for SNP IDs" >> README_summary_report

echo -e "\nTotal number of SNPs removed due to failing genomestudio stage: $number_of_snps_failed_genomestudio		...see file: '"SNP_zeroed_in_genomestudio_to_remove"' for SNP IDs" >> README_summary_report

echo -e "\nNumber of samples removed due to call rate below 98%: $number_of_samples_with_low_call_rate			...see file: '"samples_with_low_callrate_to_exclude"' for sample IDs " >> README_summary_report
...
echo -e "\nNumber of duplicate samples: $number_of_duplicate_samples		...see file: '"${report_file}.report_duplicate_sample_id_temp_changes"' for sample IDs " >> README_summary_report

echo -e "\nNumber of samples to be aware of due to heterozygote variability: $number_of_het_samples		...see file '"het_outliers_sample_exclude"' for sample IDs" >> README_summary_report

echo -e "\nNumber of related samples identified: $number_of_related_samples		...see file '"related_sample_exclude"' for sample IDs" >> README_summary_report

echo -e "\nNumber of gender missmatches identified: $number_of_gender_missmatches		...see file '"gender_missmatches"' for sample IDs" >> README_summary_report
