...
echo -e "\nNumber of samples removed due to call rate below 95%: $number_of_samples_with_low_call_rate			...see file: '"sample_information/samples_with_low_callrate_to_exclude"' for sample IDs " >> README_summary_report
...
Using only autosomal SNPs, SNPs and samples are iteratively removed which have a call rate below 0.95. Regions with LD are removed, after which samples with outlying heterozygote rates and related samples are identified for users. SNPs with a low call rate are re-introduced.
...
+ **Number of samples removed due to call rate below 95%:** $number_of_samples_with_low_call_rate
