...
exome_chip_bin="/scratch/project/pipelines/exome_chip/test/"
...
working_dir="/scratch/project/pipelines/exome_chip/test"  # PATH TO WHERE YOU WANT ALL OUTPUT
export PATH=$PATH:${exome_chip_bin}:${zcall_bin}:${opticall_bin}:${working_dir}
export data_path="/scratch/project/pipelines/exome_chip/test" # PATH TO GS REPORT FILE
manifest_file="HumanOmniExpress-12-v1-1-C.csv" # path to the manifest_file.csv: illumina manifest file for the chip type
multi_mapping_probes="multi_snp_test" # list of probes multimapping to the genome # if none supply empty list
clinical_gender="test_gender_update_file" # gender file in tab delimited format - no header - sample id tab gender
export basename="test"  # leave off suffix ".report" for basename. Report file from genomestudio
